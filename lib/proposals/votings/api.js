...
module.exports = function (api, MW, g) {
  //
  const Results = require('./results')(g)
  ...
  function _getCasts (req, res, next) {
    if (req.fetched.get('secret') === 1) {
      return next(g.createError('this is secret voting'))
    }
    if (req.fetched.get('status') !== 'locked') {
      return next(g.createError('not yet locked'))
    }
    req.params.relation = 'casts'
    req.query = {proposalid: req.params.id}
    next()
  }
  api.get('/:id/casts', MW.fetch, _getCasts, MW.fetch_rel, MW.get_rel)
  ...
  api.get('/:id/mycast', g.authMW, _getMyVoting)

  api.get('/:id/results', MW.fetch, Results.getResultsMW)
}
